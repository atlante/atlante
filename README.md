...
2. **Document values** (`{{values.project}}`): written by you (or generated by
   `init`) in `atlante.jsonc`. These are substituted into the prompt definition
...
- **`starter`** — a guide agent and a build agent, the default for bare `init`
...
Run `atlante init` without `--preset` to scaffold from the `starter` preset.
