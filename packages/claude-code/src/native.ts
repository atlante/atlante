...
const NATIVE_FORMAT = "atlante-claude-code-native" as const;
...
const DEFAULT_AGENT_OUTPUT_DIR = ".claude/agents";
const DEFAULT_SKILL_OUTPUT_DIR = ".claude/skills";

export type ClaudeCodePreparedAgent = Readonly<{
  hostAgentId: string;
  description: string;
  prompt: string;
}>;

export type ClaudeCodePreparedSkill = Readonly<{
  skillId: string;
  description: string;
  content: string;
}>;

export type ClaudeCodeOutputDirectory = Readonly<{
  outDir: string;
}>;

export type ClaudeCodeOutputOptions = Readonly<{
  agents: ClaudeCodeOutputDirectory;
  skills: ClaudeCodeOutputDirectory;
}>;
 ...
 */
export type ClaudeCodePreparedProject = Readonly<{
  /** Normalized output directories; omitted for backwards-compatible defaults. */
  options?: ClaudeCodeOutputOptions;
  agents: readonly ClaudeCodePreparedAgent[];
  skills: readonly ClaudeCodePreparedSkill[];
}>;

export type ClaudeCodeOwnedFile = Readonly<{
  kind: "agent" | "skill";
  id: string;
  path: string;
  sha256: string;
...
export type ClaudeCodeOwnershipManifest = Readonly<{
  format: typeof NATIVE_FORMAT;
  version: typeof NATIVE_VERSION;
  files: readonly ClaudeCodeOwnedFile[];
}>;

/** One verified native output returned to local consumers such as eval. */
export type ClaudeCodeNativeFile = Readonly<
  ClaudeCodeOwnedFile & { bytes: Uint8Array }
>;

/** The manifest and its verified native output files. */
export type ClaudeCodeNativeProject = Readonly<{
  manifest: ClaudeCodeOwnershipManifest;
  files: readonly ClaudeCodeNativeFile[];
}>;

export type ClaudeCodeMaterializationOperation =
  | "ensure-metadata"
  | "create-stage"
  | "stage-directory"
  | "stage-file"
...
export type ClaudeCodeMaterializerDependencies = Readonly<{
  /** Test seam for failures at filesystem boundaries. */
  fault?: (operation: ClaudeCodeMaterializationOperation, path: string) => void;
}>;

export type ClaudeCodeMaterializationResult = Readonly<{
  manifestPath: string;
  manifest: ClaudeCodeOwnershipManifest;
...
export type ClaudeCodeMaterializationErrorCode =
  | "invalid-input"
  | "invalid-id"
  | "invalid-manifest"
  | "unsafe-path"
...
export class ClaudeCodeMaterializationError extends Error {
  readonly code: ClaudeCodeMaterializationErrorCode;
  readonly path?: string;

  constructor(
    message: string,
    code: ClaudeCodeMaterializationErrorCode,
    path?: string,
    cause?: unknown,
  ) {
    super(message, { cause });
    this.name = "ClaudeCodeMaterializationError";
    this.code = code;
    this.path = path;
  }
}
...
type ValidatedPreparedProject = {
  options: ClaudeCodeOutputOptions;
  agents: ClaudeCodePreparedAgent[];
  skills: ClaudeCodePreparedSkill[];
};

type DesiredFile = {
  entry: ClaudeCodeOwnedFile;
  relativePath: string;
  absolutePath: string;
  bytes: Uint8Array;
};
...
type MaterializationPlan = {
  ...
  manifest: ClaudeCodeOwnershipManifest;
  previousFiles: Map<string, ClaudeCodeOwnedFile>;
  desired: DesiredFile[];
  writes: DesiredFile[];
  stale: FileSnapshot[];
...
function invalidInput(
  ...
  return new ClaudeCodeMaterializationError(message, "invalid-input", path);
}
...
function invalidManifest(
  ...
  return new ClaudeCodeMaterializationError(message, "invalid-manifest", path);
}
...
function unsafePath(
  ...
  return new ClaudeCodeMaterializationError(message, "unsafe-path", path);
}
...
function existingStat(path: string): Stats | undefined {
  try {
    ...
    throw new ClaudeCodeMaterializationError(
      `cannot inspect Claude Code materialization path: ${path}`,
      "filesystem",
      path,
      cause,
    );
...
function assertNativeId(value: unknown, subject: string): string {
  if (typeof value !== "string" || value.length === 0) {
    throw new ClaudeCodeMaterializationError(
      `${subject} must be a non-empty Claude Code native ID`,
      "invalid-id",
    );
  }
  if (value.length > MAX_ID_LENGTH || !NATIVE_ID_PATTERN.test(value)) {
    throw new ClaudeCodeMaterializationError(
      `${subject} "${value}" is incompatible with Claude Code native materialization; expected lowercase kebab-case ASCII of at most ${MAX_ID_LENGTH} characters, and IDs are never renamed`,
      "invalid-id",
    );
  }
  return value;
...
function validateOutputOptions(value: unknown): ClaudeCodeOutputOptions {
  if (value === undefined)
    return {
...
function absolutePath(root: string, relativePath: string): string {
  if (!isSafeRelativePath(relativePath))
    throw unsafePath(
      "Claude Code materialization path is unsafe",
      relativePath,
    );
  ...
  if (absolute !== root && !absolute.startsWith(prefix))
    throw unsafePath(
      "Claude Code materialization path escapes the project root",
      relativePath,
    );
  return absolute;
}
...
function validateParentDirectories(root: string, relativePath: string): void {
  ...
  for (const part of relativePath.split("/").slice(0, -1)) {
    ...
    if (stats.isSymbolicLink())
      throw unsafePath(
        "Claude Code materialization parent must not be a symlink",
        current,
      );
    if (!stats.isDirectory())
      throw new ClaudeCodeMaterializationError(
        `Claude Code materialization parent must be a directory: ${current}`,
        "unsafe-path",
        current,
      );
  }
...
function readSnapshot(root: string, relativePath: string): FileSnapshot {
  ...
  if (stats.isSymbolicLink())
    throw unsafePath(
      "Claude Code materialization target must not be a symlink",
      absolute,
    );
  if (!stats.isFile())
    throw new ClaudeCodeMaterializationError(
      `Claude Code materialization target must be a regular file: ${absolute}`,
      "unsafe-path",
      absolute,
    );
  try {
    ...
    throw new ClaudeCodeMaterializationError(
      `cannot read Claude Code materialization target: ${absolute}`,
      "filesystem",
      absolute,
      cause,
    );
...
function renderAgent(agent: ClaudeCodePreparedAgent): Uint8Array {
  return utf8(
    `---\nname: ${quoteFrontmatter(agent.hostAgentId)}\ndescription: ${quoteFrontmatter(agent.description)}\n---\n${agent.prompt}`,
  );
}

function renderSkill(skill: ClaudeCodePreparedSkill): Uint8Array {
  return utf8(
...
function manifestEntryIdentity(
  ...
  index: number,
  ids: Set<string>,
): Pick<ClaudeCodeOwnedFile, "kind" | "id"> {
...
function manifestEntryPath(
  value: Record<string, unknown>,
  index: number,
  identity: Pick<ClaudeCodeOwnedFile, "kind" | "id">,
  paths: Set<string>,
): string {
...
function readVerifiedNativeFile(
  ...
  if (!snapshot.exists || !snapshot.bytes) {
    throw invalidManifest(
      `owned Claude Code target is missing: ${entry.path}`,
      snapshot.absolutePath,
    );
  }
  if (sha256(snapshot.bytes) !== entry.sha256) {
    throw new ClaudeCodeMaterializationError(
      `owned Claude Code target drifted: ${entry.path}`,
      "drift",
      snapshot.absolutePath,
    );
  }
...
function trigger(
  dependencies: ClaudeCodeMaterializerDependencies,
  operation: ClaudeCodeMaterializationOperation,
  path: string,
): void {
...
function ensureDirectory(
  ...
  for (const part of relativeDirectory.split("/")) {
    ...
    if (stats) {
      if (stats.isSymbolicLink())
        throw unsafePath(
          "Claude Code materialization directory must not be a symlink",
          current,
        );
      if (!stats.isDirectory())
        throw new ClaudeCodeMaterializationError(
          `Claude Code materialization path must be a directory: ${current}`,
          "unsafe-path",
          current,
        );
      continue;
...
function assertSnapshotUnchanged(root: string, expected: FileSnapshot): void {
  ...
  if (!snapshotMatches(current, expected)) {
    throw new ClaudeCodeMaterializationError(
      `Claude Code materialization target changed during publication: ${expected.relativePath}`,
      "publication-failed",
...
function restoreSnapshot(
  ...
  if (!expectedAfterMatches(current, expectedAfter)) {
    throw new ClaudeCodeMaterializationError(
      `cannot safely roll back changed Claude Code target: ${snapshot.relativePath}`,
      "publication-failed",
      snapshot.absolutePath,
    );
  }
...
): Map<string, ClaudeCodeOwnedFile> {
  return new Map(
...
function staleEntries(
  previousFiles: ReadonlyMap<string, ClaudeCodeOwnedFile>,
...
function captureSnapshots(
  ...
  stale: readonly ClaudeCodeOwnedFile[],
...
function validateDesiredFiles(
  ...
  for (const file of desired) {
    ...
    if (current.exists && !previous)
      throw new ClaudeCodeMaterializationError(
        `unowned Claude Code target would be overwritten: ${file.relativePath}`,
        "collision",
    ...
    if (
      ...
      throw new ClaudeCodeMaterializationError(
        `owned Claude Code target drifted since the last materialization: ${file.relativePath}`,
        "drift",
        current.absolutePath,
      );
  }
...
function staleSnapshots(
  entries: readonly ClaudeCodeOwnedFile[],
  snapshots: ReadonlyMap<string, FileSnapshot>,
): FileSnapshot[] {
  ...
  for (const entry of entries) {
    ...
    if (current.exists && sha256(current.bytes as Uint8Array) !== entry.sha256)
      throw new ClaudeCodeMaterializationError(
        `stale owned Claude Code target drifted and cannot be removed: ${entry.path}`,
        "drift",
        current.absolutePath,
      );
...
function planMaterialization(
  projectRoot: string,
  input: ClaudeCodePreparedProject,
): MaterializationPlan {
  ...
  const manifest: ClaudeCodeOwnershipManifest = {
...
function createStage(
  ...
  const root = mkdtempSync(join(metadata, ".claude-code-native.stage-"));
...
function pruneEmptyStaleSkillDirectories(
  ...
  for (const file of plan.stale) {
    ...
    try {
      ...
      throw new ClaudeCodeMaterializationError(
        `cannot prune emptied Claude Code skill directory: ${file.relativePath}`,
        "filesystem",
        directory,
        cause,
      );
...
function publishPlan(
  ...
  try {
    ...
    const detail = rollbackErrors.length
      ...
      : "";
    throw new ClaudeCodeMaterializationError(
      `Claude Code native publication failed: ${errorMessage(cause)}${detail}`,
      "publication-failed",
      undefined,
      cause,
    );
...
/**
 * Plans Claude Code-native materialization without writing anything. It runs the
 * same validation, rendering, and collision/drift checks as a real build and
 * reports the paths a build would write or remove.
 */
export function planClaudeCodeMaterialization(
  projectRoot: string,
  preparedProject: ClaudeCodePreparedProject,
): ClaudeCodeMaterializationResult {
...
/**
 * Materializes a prepared-project-shaped value into Claude Code-native files.
 *
 ...
 */
export function materializeClaudeCode(
  projectRoot: string,
  preparedProject: ClaudeCodePreparedProject,
  dependencies: ClaudeCodeMaterializerDependencies = {},
): ClaudeCodeMaterializationResult {
 ...
 */
export function readClaudeCodeNative(
  projectRoot: string,
  ...
  if (!previous.manifest) {
    throw invalidManifest(
      "Claude Code ownership manifest is missing",
