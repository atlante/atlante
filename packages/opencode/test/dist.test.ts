  ...
  return [
    {
      ...
      path: `.opencode/agents/${PREPARED.agents[0].hostAgentId}.md`,
...
beforeAll(async () => {
  ...
  if (!existsSync(DIST_INDEX)) {
    throw new Error(
      `@atlante/opencode: build completed without ${relative(ROOT, DIST_INDEX)}`,
    );
  }
}, 30_000);
...
type BuiltEntry = {
  OPENCODE_HOST_TARGET: string;
  OpenCodeMaterializationError: abstract new (...args: never[]) => Error;
  materializeOpenCode: (
    projectRoot: string,
    prepared: unknown,
...
test("the built entry materializes native files and the ownership manifest", async () => {
  ...
  try {
    ...
    expect(result.manifest.format).toBe("atlante-opencode-native");
...
test("the built entry fails closed on an unowned collision", async () => {
  ...
  try {
    ...
    const { materializeOpenCode, OpenCodeMaterializationError } =
      await builtEntry();
