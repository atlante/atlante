...
// specifier under packages/cli/src to the read-only subpath; revisit this
// gate if the CLI ever legitimately needs the full contract.
test("keeps CLI imports of @atlante/artifacts pinned to the read-only subpath", () => {
  const offenders: string[] = [];
  for (const file of filesUnder(join(ROOT, "packages", "cli", "src"))) {
    const specifiers = [
